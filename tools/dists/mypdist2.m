...
switch metric
  ...
  case 'logeuc'
    D = distLogEuc(X, Y);
  case 'grass'
    D = grassmannian(X,Y, 'rbc'); % rbf binet cauchy.
  otherwise
...
%id=parfor_progress(n);
for i=1:n
  yi = Y(i,:);  yi = yi( mOnes, : );
  D(:,i) = sum( abs( X-yi),2 );
 % parfor_progress(-1, id);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function D = distLogEuc(X, Y)
d = sqrt(size(X,2)); 

for i=1:size(X,1)
    X(i,:) = vec(logm(reshape(X(i,:), [d,d])));
end
for j=1:size(Y,1)
    Y(j,:) = vec(logm(reshape(Y(j,:), [d,d])));
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function D = distStein(X, Y)
%{
    d = sqrt(size(X,2)); 

    XX = cell(size(X,1),1); 
    for i=1:size(X,1)
        XX{i} = reshape(X(i,:), [d,d]);
    end
    YY = cell(size(Y,1), 1);
    for j=1:size(Y,1)
        YY{j} = reshape(Y(j,:), [d,d]);
    end
end
D = zeros(size(X,1), size(Y,1));
%}
D = zeros(length(X), length(Y));
small = eye(size(X{1},1))*1e-5;
...
if length(X)==length(Y)    
    ...
    for i=1:length(X) 
        ...
        for j=i+1:length(X)
            D(i,j) = fast_jsld(XL{i}, XL{j}, X{i}, X{j});
        end
    end
    D = D+D';
else
    ...
    for i=1:length(X)
        if mod(i,10)==0,fprintf('.');end
        for j=1:length(Y)
            D(i,j) = fast_jsld(XL{i}, YL{j}, X{i}, Y{j});
        end
    end
end
%D = exp(-0.5*D;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function d = jsld(x, y)
small = eye(size(x,1))*1e-8;
d = log(det((x+y+small)/2)) - 0.5*log(det(x+small)) - 0.5*log(det(y+small));
...
function dist = fast_jsld(XL, YL, X, Y)
    LXY = chol((X+Y)/2);
    dist = 2*sum(log(diag(LXY))) - sum(log(diag(XL))) - sum(log(diag(YL)));

function dist = grassmannian(X,Y, type)
    dist = zeros(length(X), length(Y));
    d = size(X{1},2);
    %disp(size(X{1}));
    %myparpool(6);
    parfor i=1:length(X)
        %dist(i,:) = cellfun(@(yy) abs(1-norm(X{i}'*yy,'fro')^2/d), Y);        
        dist(i,:) = cellfun(@(yy) exp(-0.5*(1-sqrt(sum(sum((X{i}'*yy).^2)))/d)), Y); %exp(-0.5*(1-sum(sum((X{i}'*yy).^2))/d^2)), Y);                                  
        %dist(i,:) = cellfun(@(yy) exp(-0.5*(1-norm(X{i}'*yy,'fro'))/d), Y);        
    end
    %delete(gcp);